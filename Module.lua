...
function Material.Load(Config)
    ...
    local function GetExploit()
        ...
        if (pebc_create) then return "ProtoSmasher" end
        if (issentinelclosure) then return "Sentinel" end
        if (getexecutorname) then return "ScriptWare" end
        return "Undefined"
    end
    ...
    if ProtectFunctions[Exploit] then
        ProtectFunctions[Exploit](NewInstance)
    else
...
### 2. Better Example.lua
*Save this as `Example.lua`. This uses the new themes and components.*
