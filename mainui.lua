--[[ 
    Material Lua - Mobile Optimized Rewrite
    Fixed input handling for Touch & Mouse
    Added new Themes
]]
...
-- Mobile Compatible GetXY (Uses Input Object or Global Mouse)
local function GetXY(GuiObject, Input)
    local InputPosition = Input and Input.Position or UserInputService:GetMouseLocation()
	local Max, May = GuiObject.AbsoluteSize.X, GuiObject.AbsoluteSize.Y
	local Px, Py = math.clamp(InputPosition.X - GuiObject.AbsolutePosition.X, 0, Max), math.clamp(InputPosition.Y - GuiObject.AbsolutePosition.Y, 0, May)
	return Px/Max, Py/May
end

local function CircleAnim(GuiObject, EndColour, StartColour, InputObject)
	local PX, PY = GetXY(GuiObject, InputObject)
	...
	spawn(function() wait(2) Circle:Destroy() end)
end
...
local NavBar = {
	Normal = function()
		...
		NavBarContent.ChildAdded:Connect(function(Child)
			pcall(function() TweenService:Create(Child, TweenInfo.new(1), {TextTransparency = (#NavBarContent:GetChildren() - 2 > 1) and 0.5 or 0}):Play() end)
			pcall(function() TweenService:Create(Child, TweenInfo.new(1), {ImageTransparency = (#NavBarContent:GetChildren() - 2 > 1) and 0.5 or 0}):Play() end)
			pcall(function() TweenService:Create(Child:FindFirstChildWhichIsA("ImageLabel"), TweenInfo.new(1), {ImageTransparency = (#NavBarContent:GetChildren() - 2 > 1) and 0.5 or 0}):Play() end)
	...
	Invert = function()
		...
		NavBarContent.Parent = NewNavBar
		NavBarContent.ChildAdded:Connect(function(Child)
            -- Logic same as Normal but colors inverted
			pcall(function() TweenService:Create(Child, TweenInfo.new(1), {TextTransparency = (#NavBarContent:GetChildren() - 2 > 1) and 0.5 or 0}):Play() end)
			pcall(function() TweenService:Create(Child, TweenInfo.new(1), {ImageTransparency = (#NavBarContent:GetChildren() - 2 > 1) and 0.5 or 0}):Play() end)
			pcall(function() TweenService:Create(Child:FindFirstChildWhichIsA("ImageLabel"), TweenInfo.new(1), {ImageTransparency = (#NavBarContent:GetChildren() - 2 > 1) and 0.5 or 0}):Play() end)
	...
	Sheets = function()
		...
		NavBarContent.Parent = NewNavBar
		local NavBarList = Objects.new("UIListLayout")
		NavBarList.FillDirection = Enum.FillDirection.Vertical; NavBarList.HorizontalAlignment = Enum.HorizontalAlignment.Center; NavBarList.VerticalAlignment = Enum.VerticalAlignment.Top; NavBarList.SortOrder = Enum.SortOrder.LayoutOrder; NavBarList.Parent = NavBarContent
...
function Material.Load(Config)
	...
	if syn then syn.protect_gui(NewInstance); NewInstance.Parent = CoreGui
	elseif gethui then NewInstance.Parent = gethui()
	else NewInstance.Parent = CoreGui end
	...
	function TabLibrary.Banner(BannerConfig)
		...
		local BannerLabel = Objects.new("Label"); BannerLabel.Name = "Value"; BannerLabel.Text = BannerText; BannerLabel.TextColor3 = Theme.BannerAccent; BannerLabel.TextSize = 12; BannerLabel.Font = Enum.Font.Gotham; BannerLabel.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(-5,(Lines*20)+5); BannerLabel.TextWrapped = true; BannerLabel.Position = UDim2.fromOffset(5,0); BannerLabel.TextTransparency = 1; BannerLabel.ZIndex = 80; BannerLabel.Parent = Banner
		...
		for Option, Value in pairs(BannerOptions) do
			if typeof(Value) == "function" then
				...
				local OptionLabel = Objects.new("Label"); OptionLabel.Text = Option:upper(); OptionLabel.TextSize = 12; OptionLabel.TextColor3 = Theme.BannerAccent; OptionLabel.Font = Enum.Font.GothamBold; OptionLabel.Size = UDim2.fromScale(1,1); OptionLabel.Position = UDim2.fromScale(0,0); OptionLabel.TextXAlignment = Enum.TextXAlignment.Center; OptionLabel.ZIndex = 80; OptionLabel.Parent = OptionItem
	...
	function TabLibrary.New(TabConfig)
		...
		if ImageID then
			if Title then
				local TextSize = TextService:GetTextSize(Title:upper(), 12, Enum.Font.GothamBold, Vector2.new(0,0)).X
				Button = Objects.new("Button"); Button.Name = Title:upper(); Button.TextXAlignment = Enum.TextXAlignment.Right; Button.TextSize = 12; Button.Font = Enum.Font.GothamBold; Button.Text = Title:upper(); Button.Size = UDim2.fromScale(0,1) + UDim2.fromOffset(TextSize+35); Button.ZIndex = 200; Button.TextTransparency = 1
			end
			...
			local TextSize = TextService:GetTextSize(Title:upper(), 12, Enum.Font.GothamBold, Vector2.new(0,0)).X
			Button = Objects.new("Button"); Button.Name = Title:upper(); Button.TextXAlignment = Enum.TextXAlignment.Center; Button.TextSize = 12; Button.Font = Enum.Font.GothamBold; Button.Text = Title:upper(); Button.Size = UDim2.fromScale(0,1) + UDim2.fromOffset(TextSize+10); Button.ZIndex = 200; Button.TextTransparency = 1
		end
		...
		function OptionLibrary.Dropdown(DropdownConfig)
			...
			local DropdownTitle = Objects.new("Button"); DropdownTitle.Name = "Title"; DropdownTitle.Font = Enum.Font.GothamSemibold; DropdownTitle.Text = DropdownValue and DropdownText..": "..DropdownValue or DropdownText; DropdownTitle.TextColor3 = Theme.DropdownAccent; DropdownTitle.TextTransparency = 1; DropdownTitle.TextSize = 14; DropdownTitle.Parent = DropdownBar
			...
			function DropdownLibrary:SetText(Value) DropdownTitle.Text = Value end
			function DropdownLibrary:GetText() return DropdownTitle.Text end
            function DropdownLibrary:GetValue() return DropdownValue end
            -- (SetOptions function omitted for brevity, logic follows CreateNewButton pattern)
			return DropdownLibrary
		end

		function OptionLibrary.ChipSet(ChipSetConfig)
            -- Chipset Logic (Simplified for mobile context: Buttons already handled by CreateNewButton/InputBegan logic internally if rewritten, 
            -- but ChipSet creates custom buttons. We apply the input fix here.)
			...
			for _ in pairs(ChipSetOptions) do TotalOptions = TotalOptions + 1 end

			if TotalOptions > 0 then
				local Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,(TotalOptions*30)+((TotalOptions+1)*5))
				local ChipSet = Objects.new("Round"); ChipSet.Name = "ChipSet"; ChipSet.Size = Size; ChipSet.ImageColor3 = Theme.ChipSet; ChipSet.ImageTransparency = 1; ChipSet.Parent = PageContentFrame
				local ChipList = Objects.new("UIListLayout"); ChipList.SortOrder = Enum.SortOrder.LayoutOrder; ChipList.Padding = UDim.new(0,5); ChipList.Parent = ChipSet
				local ChipPadding = Objects.new("UIPadding"); ChipPadding.PaddingBottom = UDim.new(0,5); ChipPadding.PaddingTop = UDim.new(0,5); ChipPadding.PaddingRight= UDim.new(0,5); ChipPadding.PaddingLeft = UDim.new(0,5); ChipPadding.Parent = ChipSet

				local BuildTable = {}
				for Key, Value in pairs(ChipSetOptions) do
					if typeof(Value) == "table" then BuildTable[Key] = Value.Enabled else BuildTable[Key] = Value end
				end
				ChipSetCallback(BuildTable)
				TweenService:Create(ChipSet, TweenInfo.new(0.5), {ImageTransparency = 0.9}):Play()

				for Key, Value in pairs(ChipSetOptions) do
					local ChipItem = Objects.new("SmoothButton"); ChipItem.Name = "ChipItem"; ChipItem.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,30); ChipItem.ImageColor3 = BuildTable[Key] and Theme.ChipSet or Theme.ChipSetAccent; ChipItem.ImageTransparency = 1; ChipItem.Parent = ChipSet
					local ChipShadow = Objects.new("Shadow"); ChipShadow.ImageColor3 = BuildTable[Key] and Theme.ChipSet or Theme.ChipSetAccent; ChipShadow.ImageTransparency = 1; ChipShadow.Parent = ChipItem
					local Tick = Objects.new("Round"); Tick.ScaleType = Enum.ScaleType.Stretch; Tick.Image = "http://www.roblox.com/asset/?id=5554953789"; Tick.ImageColor3 = Theme.ChipSetAccent; Tick.ImageTransparency = 1; Tick.Size = UDim2.fromScale(1,1) - UDim2.fromOffset(10,10); Tick.SizeConstraint = Enum.SizeConstraint.RelativeYY; Tick.Position = UDim2.fromOffset(5,5); Tick.Parent = ChipItem
					local ChipLabel = Objects.new("Label"); ChipLabel.Size = BuildTable[Key] and (UDim2.fromScale(1,1) - UDim2.fromOffset(30)) or (UDim2.fromScale(1,1) - UDim2.fromOffset(5)); ChipLabel.Position = BuildTable[Key] and UDim2.fromOffset(30) or UDim2.fromOffset(5); ChipLabel.Text = Key; ChipLabel.Font = Enum.Font.Gotham; ChipLabel.TextSize = 12; ChipLabel.TextColor3 = BuildTable[Key] and Theme.ChipSetAccent or Theme.ChipSet; ChipLabel.TextTransparency = 1; ChipLabel.Parent = ChipItem
					...
					ChipItem.InputBegan:Connect(function(Input)
						if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
							...
							if ChipMenu then TweenService:Create(ChipMenu, TweenInfo.new(0.15), {ImageColor3 = Enabled and Theme.ChipSetAccent or Theme.ChipSet}):Play() end
							ChipSetCallback(BuildTable)
						end
					end)
				end
				return { GetOptions = function() return ChipSetOptions end }
			end
		end

        -- DataTable omitted for brevity, acts exactly like Chipset
		...
		function OptionLibrary.ColorPicker(ColorPickerConfig)
			...
			local ColorPicker = Objects.new("SmoothButton"); ColorPicker.Name = "ColorPicker"; ColorPicker.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,40); ColorPicker.ImageColor3 = Theme.ColorPicker; ColorPicker.ImageTransparency = 1; ColorPicker.ClipsDescendants = true; ColorPicker.Parent = PageContentFrame
			...
			local ColorBar = Objects.new("Round"); ColorBar.Name = "TitleBar"; ColorBar.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(-10,30); ColorBar.Position = UDim2.fromOffset(5,5); ColorBar.ImageColor3 = Theme.ColorPickerAccent; ColorBar.ImageTransparency = 1; ColorBar.Parent = ColorPicker
			local ColorShadow = Objects.new("Shadow"); ColorShadow.ImageColor3 = Theme.ColorPickerAccent; ColorShadow.ImageTransparency = 1; ColorShadow.Parent = ColorBar
			local ColorLabel = Objects.new("Label"); ColorLabel.Name = "Title"; ColorLabel.Font = Enum.Font.GothamSemibold; ColorLabel.TextColor3 = Theme.ColorPickerAccent; ColorLabel.TextSize = 14; ColorLabel.Text = ColorPickerText; ColorLabel.TextTransparency = 1; ColorLabel.Parent = ColorBar
			local ColorTracker = Objects.new("Round"); ColorTracker.Name = "Tracker"; ColorTracker.Size = UDim2.fromOffset(50,20); ColorTracker.Position = UDim2.fromScale(1,0) + UDim2.fromOffset(-55,5); ColorTracker.ImageColor3 = ColorPickerDefault; ColorTracker.ImageTransparency = 1; ColorTracker.Parent = ColorBar
			local TrackerShadow = Objects.new("Shadow"); TrackerShadow.ImageColor3 = ColorTracker.ImageColor3; TrackerShadow.ImageTransparency = 1; TrackerShadow.Parent = ColorTracker
			ColorTracker:GetPropertyChangedSignal("ImageColor3"):Connect(function() TrackerShadow.ImageColor3 = ColorTracker.ImageColor3 end)

            -- Pickers construction
			local Hue = Objects.new("Round"); Hue.Name = "Container"; Hue.Position = UDim2.fromOffset(5,40); Hue.Size = UDim2.fromOffset(25,20); Hue.ImageColor3 = Theme.ColorPickerAccent; Hue.ImageTransparency = 0.8; Hue.Parent = ColorPicker
			local Saturation = Hue:Clone(); Saturation.Position = Saturation.Position + UDim2.fromOffset(0,25); Saturation.Parent = ColorPicker
			local Value = Saturation:Clone(); Value.Position = Value.Position + UDim2.fromOffset(0,25); Value.Parent = ColorPicker
            -- Gradients omitted for brevity, same as original
            local HueTracker = Objects.new("SmoothButton"); HueTracker.Name = "Tracker"; HueTracker.ImageColor3 = Color3.fromRGB(255,255,255); HueTracker.ImageTransparency = 1; HueTracker.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(-40,20); HueTracker.Position = UDim2.fromOffset(35,40); HueTracker.Parent = ColorPicker
            ...
            H:GetPropertyChangedSignal("Value"):Connect(UpdateColor)
            S:GetPropertyChangedSignal("Value"):Connect(UpdateColor)
            V:GetPropertyChangedSignal("Value"):Connect(UpdateColor)

            --// COLOR DRAGGING FIX (MOBILE) //--
            local function HandleInput(Obj, ValueObj)
                local DraggingColor = false
                
                local function Update(Input)
                    ValueObj.Value = 1 - GetXY(Obj, Input)
                end
                ...
                Obj.InputBegan:Connect(function(Input)
                    if Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch then
                        ...
                        Connection = UserInputService.InputChanged:Connect(function(Input2)
                            if (Input2.UserInputType == Enum.UserInputType.MouseMovement or Input2.UserInputType == Enum.UserInputType.Touch) and DraggingColor then
                                Update(Input2)
                            end
                        end)
            ...
            HandleInput(HueTracker, H)
            HandleInput(SaturationTracker, S)
            HandleInput(ValueTracker, V)
            --// END COLOR DRAGGING //--
            
            local MenuAdded, MenuButton = TryAddMenu(ColorLabel, ColorPickerMenu, {})
			if MenuAdded then ColorTracker.Position = ColorTracker.Position - UDim2.fromOffset(25,0); MenuButton.ImageColor3 = Theme.ColorPickerAccent end

			return { SetColor = function(_, Val) H.Value,S.Value,V.Value = Color3.toHSV(Val) end, GetColor = function() return ColorTracker.ImageColor3 end }
		end
		...
		function OptionLibrary.Toggle(ToggleConfig)
			...
			local Toggle = Objects.new("SmoothButton"); Toggle.Name = "Toggle"; Toggle.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,30); Toggle.ImageColor3 = Theme.Toggle; Toggle.ImageTransparency = 1; Toggle.Parent = PageContentFrame
			local ToggleTracker = Objects.new("Round"); ToggleTracker.Name = "Tracker"; ToggleTracker.Image = "rbxassetid://3570695787"; ToggleTracker.SliceCenter = Rect.new(100,100,100,100); ToggleTracker.SliceScale = 1; ToggleTracker.Size = UDim2.fromOffset(26,12); ToggleTracker.Position = UDim2.fromScale(1,0.5) - UDim2.fromOffset(41,6); ToggleTracker.ImageColor3 = Theme.Toggle; ToggleTracker.ImageTransparency = 1; ToggleTracker.Parent = Toggle
			local Dot = Objects.new("Circle"); Dot.Name = "Dot"; Dot.Size = UDim2.fromOffset(16,16); Dot.Position = UDim2.fromScale(0,0.5) - UDim2.fromOffset(8,8); Dot.ImageColor3 = Theme.ToggleAccent; Dot.ImageTransparency = 1; Dot.Parent = ToggleTracker
			local DotShadow = Objects.new("Round"); DotShadow.Name = "Shadow"; DotShadow.Image = "http://www.roblox.com/asset/?id=5554831957"; DotShadow.ScaleType = Enum.ScaleType.Stretch; DotShadow.Size = UDim2.fromOffset(31,31); DotShadow.Position = UDim2.fromOffset(-8,-8); DotShadow.ImageColor3 = Theme.Toggle; DotShadow.ImageTransparency = 1; DotShadow.Parent = Dot
			local ToggleLabel = Objects.new("Label"); ToggleLabel.Font = Enum.Font.GothamSemibold; ToggleLabel.TextSize = 14; ToggleLabel.Text = ToggleText; ToggleLabel.TextColor3 = Theme.Toggle; ToggleLabel.TextTransparency = 1; ToggleLabel.ClipsDescendants = true; ToggleLabel.Parent = Toggle
			...
			if MenuAdded then ToggleTracker.Position = ToggleTracker.Position - UDim2.fromOffset(15,0); MenuButton.ImageColor3 = Theme.Toggle end

			return { SetState = function(_, Val) ToggleDefault = Val; TweenService:Create(Dot, TweenInfo.new(0.15), {Position = (ToggleDefault and UDim2.fromScale(1,0.5) or UDim2.fromScale(0,0.5)) - UDim2.fromOffset(8,8), ImageColor3 = ToggleDefault and Theme.Toggle or Theme.ToggleAccent}):Play(); ToggleCallback(ToggleDefault) end, GetState = function() return ToggleDefault end }
		end

		function OptionLibrary.TextField(TextFieldConfig)
            -- Logic matches original but uses input services for focus if needed, standard TextBox usually handles mobile fine
			...
			local TextField = Objects.new("Round"); TextField.Name = "TextField"; TextField.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,30); TextField.ImageColor3 = Theme.TextField; TextField.ImageTransparency = 1; TextField.Parent = PageContentFrame
			local TextEffect = Objects.new("Frame"); TextEffect.Name = "Effect"; TextEffect.BackgroundTransparency = 1; TextEffect.BackgroundColor3 = Theme.TextField; TextEffect.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,2); TextEffect.Position = UDim2.fromScale(0,1) - UDim2.fromOffset(0,2); TextEffect.Parent = TextField
			local TextShadow = Objects.new("Shadow"); TextShadow.ImageColor3 = Theme.TextField; TextShadow.ImageTransparency = 1; TextShadow.Parent = TextField
			local TextInput = Objects.new("Box"); TextInput.Name = "Value"; TextInput.PlaceholderText = TextFieldText; TextInput.PlaceholderColor3 = Theme.TextFieldAccent; TextInput.TextInputType = Enum.TextInputType[TextFieldInputType]; TextInput.TextColor3 = Theme.TextFieldAccent; TextInput.Text = ""; TextInput.Font = Enum.Font.GothamSemibold; TextInput.TextSize = 14; TextInput.TextTransparency = 1; TextInput.Parent = TextField
			...
			TextInput.Focused:Connect(function() TweenService:Create(TextField, TweenInfo.new(0.5), {ImageTransparency = 0.7}):Play(); TweenService:Create(TextInput, TweenInfo.new(0.5), {TextTransparency = 0}):Play() end)
			TextInput.FocusLost:Connect(function() TweenService:Create(TextField, TweenInfo.new(0.5), {ImageTransparency = 0.8}):Play(); TweenService:Create(TextInput, TweenInfo.new(0.5), {TextTransparency = 0.5}):Play(); TextFieldCallback(TextInput.Text) end)

			local MenuAdded, MenuBar = TryAddMenu(TextField, Menu, { SetText = function(Value) TextInput.Text = Value; TextFieldCallback(TextInput.Text) end })
			if MenuAdded then MenuBar.ImageColor3 = Theme.TextFieldAccent end
			return { SetText = function(_,Val) TextInput.Text = Val end, GetText = function() return TextInput.Text end }
		end

		function OptionLibrary.Label(LabelConfig)
			local LabelText = LabelConfig.Text or "nil label"
			local LabelContainer = Objects.new("Round"); LabelContainer.Name = "Label"; LabelContainer.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,20); LabelContainer.ImageColor3 = Theme.MainFrame; LabelContainer.Parent = PageContentFrame
			local LabelContent = Objects.new("Label"); LabelContent.TextColor3 = Theme.ChipSet; LabelContent.Text = LabelText:upper(); LabelContent.TextSize = 12; LabelContent.Font = Enum.Font.GothamSemibold; LabelContent.Size = UDim2.fromScale(1,1) + UDim2.fromOffset(-5,0); LabelContent.Position = UDim2.fromOffset(5,0); LabelContent.Parent = LabelContainer
			return { SetText = function(Text) LabelContent.Text = Text end }
		end
		...
		function OptionLibrary.Slider(SliderConfig)
			...
			if SliderMin > SliderMax then local ValueBefore = SliderMin; SliderMin, SliderMax = SliderMax, ValueBefore end
			...
			local Slider = Objects.new("Round"); Slider.Name = "Slider"; Slider.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(0,35); Slider.ImageColor3 = Theme.Slider; Slider.ImageTransparency = 1; Slider.Parent = PageContentFrame
			local SliderShadow = Objects.new("Shadow"); SliderShadow.ImageColor3 = Theme.Slider; SliderShadow.ImageTransparency = 1; SliderShadow.Parent = Slider
			local SliderTitle = Objects.new("Label"); SliderTitle.TextColor3 = Theme.SliderAccent; SliderTitle.Text = SliderText; SliderTitle.TextSize = 14; SliderTitle.Font = Enum.Font.GothamSemibold; SliderTitle.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(-5,25); SliderTitle.TextTransparency = 1; SliderTitle.Parent = Slider
			local SliderValue = Objects.new("Label"); SliderValue.Text = tostring(SliderDef); SliderValue.TextColor3 = Theme.SliderAccent; SliderValue.TextTransparency = 1; SliderValue.TextSize = 14; SliderValue.Size = UDim2.fromScale(1,0) + UDim2.fromOffset(-5,25); SliderValue.Position = UDim2.fromScale(0,0); SliderValue.TextXAlignment = Enum.TextXAlignment.Right; SliderValue.Font = Enum.Font.GothamSemibold; SliderValue.Parent = Slider
			...
			local SliderFill = SliderTracker:Clone(); SliderFill.BackgroundTransparency = 1; SliderFill.Position = UDim2.fromScale(0,0); SliderFill.Size = UDim2.fromScale(DefaultScale,1); SliderFill.Parent = SliderTracker
			local MinSize = 10; local MaxSize = 36
			local SizeFromScale = (MinSize +  (MaxSize - MinSize)) * DefaultScale
			SizeFromScale = SizeFromScale - (SizeFromScale % 2)
			local SliderDot = Objects.new("CircleButton"); SliderDot.Size = UDim2.fromOffset(10,10); SliderDot.Position = UDim2.fromScale(DefaultScale,0.5) - UDim2.fromOffset(5,5); SliderDot.ImageColor3 = Theme.SliderAccent; SliderDot.ImageTransparency = 1; SliderDot.ZIndex = 50; SliderDot.Parent = SliderTracker
			local SliderFadedDot = Objects.new("Circle"); SliderFadedDot.Size = UDim2.fromOffset(SizeFromScale,SizeFromScale); SliderFadedDot.Position = UDim2.fromScale(0.5,0.5) - UDim2.fromOffset(SizeFromScale/2,SizeFromScale/2); SliderFadedDot.ImageColor3 = Theme.SliderAccent; SliderFadedDot.ImageTransparency = 1; SliderFadedDot.ZIndex = 50; SliderFadedDot.Parent = SliderDot
            ...
            --// SLIDER DRAGGING FIX (MOBILE) //--
            local DraggingSlider = false
            local function UpdateSlider(Input)
                local Px = GetXY(SliderTracker, Input)
			...
			if MenuAdded then SliderValue.Position = SliderValue.Position - UDim2.fromOffset(25,0); SliderTracker.Size = SliderTracker.Size - UDim2.fromOffset(20,0); MenuButton.ImageColor3 = Theme.SliderAccent end
			return { SetValue = function(_, Val)
                -- Simplistic logic to update visual
                local Px = math.clamp((Val - SliderMin) / (SliderMax - SliderMin), 0, 1)
                TweenService:Create(SliderDot, TweenInfo.new(0.15), {Position = UDim2.fromScale(Px,0.5) - UDim2.fromOffset(5,5)}):Play()
                TweenService:Create(SliderFill, TweenInfo.new(0.15), {Size = UDim2.fromScale(Px, 1)}):Play()
                SliderValue.Text = tostring(Val)
            end, GetValue = function() return tonumber(SliderValue.Text) end }
		end
