...
**Changes made:**
1.  **Frame Dragging:** Rewritten using `UserInputService` to properly handle Mobile Touch and Mouse Dragging without getting stuck.
2.  **Sliders & Color Pickers:** Rewritten the input logic. It now uses a "Global Disconnect" method. When you start clicking/touching, it creates a temporary connection to track movement. As soon as you lift your finger or mouse button (anywhere on the screen), it disconnects immediately. This fixes the "sticking" issue.
3.  **Animations:** Sped up animations (0.5s -> 0.3s, 1s -> 0.6s) for a snappier feel.
4.  **Example Script:** Updated at the bottom with `SizeX = 170` and `SizeY = 180`.
...
**And the updated Example Script:**

```lua
local Material = loadstring(game:HttpGet("https://raw.githubusercontent.com/TwichyPlayer/MaterialLuaMobile/refs/heads/main/Module.lua"))()
